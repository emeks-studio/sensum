/*
 * This file is the main driver for the Midnight sensum example.
...
import {
  type SensumContract,
  type SensumProviders,
  type DeployedSensumContract,
  type PrivateStates,
...
import {
  ...
  Contract,
  type Ledger,
  createSensumPrivateState,
} from 'sensum-contract';
import {
...
/* **********************************************************************
 * getSensumLedgerState: a helper that queries the current state of
 * the data on the ledger, for a specific sensum contract.
 ...
 * in the sensum contract.
 */

export const getSensumLedgerState = (
  providers: SensumProviders,
...
/* **********************************************************************
 * join: find an already-deployed sensum contract on the
 * network and join it.  The user is prompted for the existing
 * contract's address.
 */

const join = async (providers: SensumProviders, rli: Interface, logger: Logger): Promise<DeployedSensumContract> => {
  const contractAddress = await rli.question('What is the contract address (in hex)? ');
  const existingPrivateState = await providers.privateStateProvider.get('sensumPrivateState');
  // FIXME: What a fuck? providers.walletProvider.coinPublicKey? I don't get it!
  ...
  logger.info(`Joined the contract at address: ${deployedContract.finalizedDeployTxData.contractAddress}`);
  return deployedContract;
};

/* **********************************************************************
 * deploy: deploy a brand new sensum contract to the network.
 */
...
You can do one of the following:
  1. Deploy a new sensum contract
  2. Join an existing sensum contract
  3. Exit
...
const deployOrJoin = async (
  ...
  rli: Interface,
  logger: Logger,
): Promise<DeployedSensumContract | null> => {
  while (true) {
 ...
 * by the contract to be in the ledger state of sensum.
 */

const displayLedgerState = async (
  providers: SensumProviders,
  deployedContract: DeployedSensumContract,
  logger: Logger,
): Promise<void> => {
  const contractAddress = deployedContract.finalizedDeployTxData.contractAddress;
  const ledgerState = await getSensumLedgerState(providers, contractAddress);
  if (ledgerState === null) {
    logger.info(`There is no sensum contract deployed at ${contractAddress}`);
  } else {
    // TODO: Display missing info:
...
const displayPrivateState = async (providers: SensumProviders, logger: Logger): Promise<void> => {
  const privateState = await providers.privateStateProvider.get('sensumPrivateState');
  if (privateState === null) {
    logger.info(`There is no existing sensum private state`);
  } else {
...
/* **********************************************************************
 * mainLoop: the main interactive menu of the sensum CLI.
...
/* **********************************************************************
 * run: the main entry point that starts the whole CLI.
 *
  ...
  try {
    if (wallet !== null) {
      ...
      const providers = {
        ...
        zkConfigProvider: new NodeZkConfigProvider<'praise_the_sun' | 'cosmic_ballet' | 'transmit' | 'im_blessed'>(config.zkConfigPath),
